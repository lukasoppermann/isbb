...
class Start extends MY_Controller {
	...
	function index()
	{
		...
		if(!isset($page) || $page == "home")
		{
			// weiterbildung
			$this->entries->get(2);
			$weiterbildung = "<div><h2>".$this->entries->get_element('title', 2)."</h2>";
			...
			// philosophie
			$this->entries->get(21);
			$philosophie = "<div><h2>".$this->entries->get_element('title', 21)."</h2>";
			...
			if(isset($news) && !empty($news))
			{
				...
				{
					if((strtotime($value['date']) + 90 * 24 * 60 * 60) > time())
					{
						if( trim($value['content']) !== "" )
						{
							$_news .= "<a href='".lang_url(TRUE)."isbb/news/".urlencode(strtolower(replace_accents(trim($value['title'])))).'/'.$value['id']."'><h3>".$value['title']."</h3>";
							$_news .= "<p>".$value['excerpt']."</p></a>";
						}
						else
						{
							$_news .= "<h3>".$value['title']."</h3>";
							$_news .= "<p>".$value['excerpt']."</p>";
						}
					}
				}
				$_news .= "</div>";
			...
			$data['page'] .= '<div class="box left">'.$weiterbildung.$philosophie.'</div>'.$_news;
