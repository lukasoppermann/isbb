...
class Team extends MY_Controller {
	...
	function index( $subpage = null )
	{
		if( $subpage === 'koeln' )
		{
			css_add('screen','team_koeln');
			view('custom/team-koeln', $this->data);
		}
		else
		{
			// get data (menus, etc.)
			...
			// get data
			$array = get_db_data('projects', $db = array('select' => '*', 'where' => array('status' => 1), 'order' => 'id'));
			shuffle($array);
			// build page
			$page = NULL;
